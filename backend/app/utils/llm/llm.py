...
async def ask_llm(msg, session_id, emit_fn=None):
    ...
    if LLM_PROVIDER == "gemini":
        ...
        response = model.chat.completions.create(model=OPENAI_MODEL_NAME, messages=[{"role": "user", "content": full_prompt}])
    ...
    if "query_request" in reply:
        ...
        # [Optimization] รัน Retrieval ใน Thread แยกเพื่อไม่ให้บล็อก Async Loop
        ...
        if LLM_PROVIDER == "gemini":
            ...
            response = model.chat.completions.create(model=OPENAI_MODEL_NAME, messages=[{"role": "user", "content": prompt_for_answer}])
            reply = response.choices[0].message.content.strip()
