...
async def _resolve_maybe_async(value):
    """Accept both sync and async return values."""
...
async def ask_llm(
    ...
    async with llm_semaphore:
        ...
        if faq_lookup_enabled:
            faq_hit = await _resolve_maybe_async(
                get_faq_answer(
                    msg,
                    include_meta=True,
                )
        ...
        try:
            ...
            if should_retrieve and top_chunks and bool(faq_cfg.get("auto_learn", True)):
                ...
                faq_update_result = await _resolve_maybe_async(update_faq(msg, reply, learn_meta))
                step_finish(
                    faq_step,
