                ...
                <div class="p-4 flex flex-col md:flex-row gap-4">
                    ...
                    <div class="flex items-center gap-2 shrink-0 flex-wrap">
                        ...
                        <button 
                            ...
                            <span class="hidden lg:inline" x-text="processing ? 'Processing...' : 'Auto PDF→TXT'"></span>
                    ...
                    <div class="flex items-center gap-3">
                        ...
                        <span class="font-bold" style="color: var(--text-primary);">
                            <template x-if="clipboard.paths.length > 0">
                                <span>
                                    <span x-text="clipboard.action === 'copy' ? 'คัดลอกแล้ว' : 'ตัดแล้ว'"></span>
                                    <span x-text="clipboard.paths.length"></span> รายการ
                                </span>
                            </template>
                            <template x-if="clipboard.paths.length === 0">
                                <span>
    ...
    return {
        ...
        async processRAG() {
            if (this.fileSystem.root !== 'data') {
                alert('กรุณาเปลี่ยนไปที่ PDF Files ก่อนทำการแปลงไฟล์');
                return;
            }
            
            if (!confirm('ต้องการแปลงไฟล์ PDF ทั้งหมดเป็น TXT สำหรับระบบ RAG?\n\nกระบวนการนี้อาจใช้เวลาหลายนาที')) {
                return;
            }
            ...
            try {
                ...
                if (response.ok) {
                    alert('✅ แปลงไฟล์ PDF เป็น TXT สำเร็จ!\n\nระบบ RAG พร้อมใช้งานแล้ว');
                } else {
                    throw new Error('Process failed');
                }
            } catch(e) { 
                alert('❌ การแปลงไฟล์ล้มเหลว\n\nกรุณาลองใหม่อีกครั้ง'); 
