/**
 * Real-Time Monitor Module โ€” Admin Live Activity Dashboard
...
function initMonitorModule(app) {
    ...
    function render() {
        ...
        const feedHtml = state.liveFeed.length === 0
            ? `<div class="p-8 text-center"><p class="text-sm font-medium" style="color: var(--text-tertiary);">เธฃเธญเธเนเธญเธเธงเธฒเธก...</p></div>`
            : state.liveFeed.map(item => {
                const isUser = item.type === 'user';
                const icon = isUser ? '๐‘ค' : '๐ค–';
                const label = isUser ? (item.name || 'User') : 'Bot เธเธตเนเน€เธฃเนเธ';
                ...
                const textPreview = (item.text || '').replace(/\[Bot เธเธตเนเน€เธฃเนเธ\]\s*/g, '').replace(/\[Admin\]:\s*/g, '');
